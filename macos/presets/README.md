...
- 该精选预设是单独记录的发行例外，不纳入 MIT 软件许可；文件清单和限制见 [`../NOTICE.md`](../NOTICE.md)。这不表示以后可以提交其他可识别真人素材。
...
内置预设会随仓库分发，**不是**「个人本地示意」。为避免给贡献者和使用者带来法律风险，只接受：
...
除非已经完成独立权利审核并在 `NOTICE.md` 逐项记录，否则**不接受**（PR 会被拒绝）：
