...
该素材作为源图、可切换预设和实机预览随仓库收录；这只记录素材指向与收录状态，不构成肖像、生成或再分发授权声明。`macos/NOTICE.md` 已将相关文件排除在 MIT 软件许可之外，使用者与下游分发者仍需独立核验相应权利。下面的公共可复制提示词继续使用原创虚构成年人物。
...
路径分类不代表肖像或再分发许可。当前 `preset-arina-hashimoto` 含用户提供的真人/AI 肖像素材；仓库公开包含这些文件并不授予或证明肖像、生成、商业使用或再次分发权利，具体边界以 `macos/NOTICE.md` 为准。
