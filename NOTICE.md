# Notices

Codex Skin Kit is an unofficial Codex desktop skin project. It uses and references
open-source work from
[Fei-Away/Codex-Dream-Skin](https://github.com/Fei-Away/Codex-Dream-Skin).
It is not affiliated with, endorsed by, or sponsored by OpenAI.

## Software license

The MIT License in [`LICENSE`](./LICENSE) applies to the software source code
in this repository, except where third-party notices state otherwise.

It does not grant rights to OpenAI or Codex trademarks, product names, logos,
trade dress, official application binaries, or user-supplied artwork.

## Third-party and upstream material

- Upstream project: `Fei-Away/Codex-Dream-Skin`
- Upstream license: MIT

## Support service

The `ttflows 天梯流` section in the README is independent from skin
functionality. Theme installation does not require ttflows and does not change
API configuration.
